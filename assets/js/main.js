$(document).ready(function () {
  ...
  getExams(document.documentElement.lang);
...
function addExams(exams) {
  for (exam of exams) {
    $(".exams").append(
      `<a class="exam-btn" href="#" onclick="updateModal('${exam}')"><span class='exam'>${removeExtension(
        snakeToCapitalize(exam)
      )}</span><a>`
    );
  }
}
...
function updateModal(exam) {
  ...
  $(".modal-actions > a.simulation").attr(
    "href",
    `introduction.html?exam=${exam}`
  );
...
function removeOldExams() {
  $(".exams").empty();
}

function setLocale() {
  let optionValues = $(".label-select img")
    .map((i, o) => o.title)
    .toArray();
